...
import game_over_screen  # <-- NOWY IMPORT
...
# --- Zmienne Stanu ---
game_state = "MENU_GLOWNE"  # Możliwe stany: MENU_GLOWNE, GETTING_PLAYER_NAMES, INSTRUCTIONS, GAMEPLAY, GAME_OVER
...
points_to_animate = 0
delay_timer = 0.0
winner_data = None  # Słownik z danymi o zwycięzcy

def set_main_screen(main_screen_surface): global screen; screen = main_screen_surface
def change_game_state(new_state, data=None):
    ...
    if new_state == "GAME_OVER":
        winner_data = data
        game_over_screen.setup_game_over_screen(current_screen_width, current_screen_height, winner_data.get("name"), winner_data.get("reason"))
    if new_state == "GAMEPLAY" and (current_screen_width != settings.GAMEPLAY_SCREEN_WIDTH or current_screen_height != settings.GAMEPLAY_SCREEN_HEIGHT):
        set_screen_mode(settings.GAMEPLAY_SCREEN_WIDTH, settings.GAMEPLAY_SCREEN_HEIGHT)
    elif new_state in ["MENU_GLOWNE", "INSTRUCTIONS", "GETTING_PLAYER_NAMES", "GAME_OVER"] and (current_screen_width != settings.INITIAL_SCREEN_WIDTH or current_screen_height != settings.INITIAL_SCREEN_HEIGHT):
        set_screen_mode(settings.INITIAL_SCREEN_WIDTH, settings.INITIAL_SCREEN_HEIGHT)
    else:
        if new_state == "GETTING_PLAYER_NAMES": import name_input_screen; name_input_screen.setup_name_input_screen(current_screen_width, current_screen_height)
...
def set_screen_mode(width, height):
    ...
    if game_state == "MENU_GLOWNE": menu_screen.load_menu_resources(current_screen_width, current_screen_height); menu_screen.setup_menu_ui_elements(current_screen_width, current_screen_height)
    elif game_state == "GAMEPLAY": initialize_gameplay()
    elif game_state == "INSTRUCTIONS": instructions_screen.load_instructions_resources(current_screen_width, current_screen_height); instructions_screen.setup_instructions_ui(current_screen_width, current_screen_height, menu_screen.BUTTON_FONT_MENU)
    elif game_state == "GETTING_PLAYER_NAMES": import name_input_screen; name_input_screen.setup_name_input_screen(current_screen_width, current_screen_height)

def initialize_gameplay():
    global final_player_names; print("Rozpoczynam inicjalizację rozgrywki...")
    question_manager.reset_available_questions(); gameplay_screen.load_gameplay_resources(current_screen_width, current_screen_height)
    gameplay_screen.setup_gameplay_ui_elements(current_screen_height, final_player_names); initialize_game_logic_and_pawns()

def start_new_game(player_names_from_input):
    global final_player_names; final_player_names = [name.strip() if name.strip() else f"Gracz {i+1}" for i, name in enumerate(player_names_from_input)]
    print(f"Ostateczne imiona graczy: {final_player_names}"); change_game_state("GAMEPLAY")

def add_points_to_player(player_id, points_to_change):
    global player_points; current_points = player_points.get(player_id, 0); actual_change = points_to_change
    if current_points + points_to_change < 0: actual_change = -current_points
    player_points[player_id] += actual_change; return actual_change
...
def trigger_points_animation(player_id, points_value_to_show):
    ...
    try:
        ...
        if 0 <= player_index < len(gameplay_screen.player_widgets): gameplay_screen.player_widgets[player_index].start_points_animation(points_value_to_show)
    except (IndexError, ValueError) as e: print(f"Błąd przy uruchamianiu animacji punktów dla ID '{player_id}': {e}")

def update_player_widgets_data():
    if gameplay_screen.player_widgets:
        for widget in gameplay_screen.player_widgets:
            widget.points = player_points.get(widget.player_id, 0); widget.lives = player_lives.get(widget.player_id, 0)

def update_active_pawn_indicator():
    for widget in gameplay_screen.player_widgets: widget.set_active(widget.player_id == current_player_id)
    for pawn in all_pawn_objects: pawn.set_active(pawn.pawn_id == current_player_id)
...
def initialize_game_logic_and_pawns():
    ...
    professor_move_pending = 0; global_turn_counter = 0; points_to_animate = 0
    player_points = {"player_1": 0, "player_2": 0}; player_lives = {"player_1": settings.INITIAL_PLAYER_LIVES, "player_2": settings.INITIAL_PLAYER_LIVES}
    update_player_widgets_data()
    if gameplay_screen.game_board_instance:
        try:
            player1 = PlayerPawn(1, settings.IMAGE_PATH_PLAYER1_PAWN, 0, gameplay_screen.game_board_instance); add_pawn_to_field_map(player1, 0); all_sprites_group.add(player1); all_pawn_objects.append(player1)
            player2 = PlayerPawn(2, settings.IMAGE_PATH_PLAYER2_PAWN, 0, gameplay_screen.game_board_instance); add_pawn_to_field_map(player2, 0); all_sprites_group.add(player2); all_pawn_objects.append(player2)
            professor = ProfessorPawn(settings.IMAGE_PATH_PROFESSOR_PAWN, 0, gameplay_screen.game_board_instance); add_pawn_to_field_map(professor, 0); all_sprites_group.add(professor); all_pawn_objects.append(professor)
            start_pawns_repositioning_on_field(0); update_active_pawn_indicator(); print("Logika gry, pionki, punkty i życia zainicjalizowane.")
...
def move_professor(steps=1):
    ...
    if professor and gameplay_screen.game_board_instance:
        total_fields = gameplay_screen.game_board_instance.get_total_fields(); old_prof_index = professor.board_field_index
        path = [(old_prof_index + i + 1) % total_fields for i in range(steps)]
        if not path: turn_phase = 'PROCESSING_AFTER_ACTION'; return
        professor_that_was_moving = professor; professor_that_was_moving.start_move_animation(path.copy())
        new_prof_index = path[-1]; remove_pawn_from_field_map(professor, old_prof_index); add_pawn_to_field_map(professor, new_prof_index)
        turn_phase = 'WAITING_FOR_PROFESSOR_MOVE'
        print(f"Profesor rozpoczyna ruch na pole {new_prof_index}")

def check_professor_capture():
    global player_lives; professor = next((p for p in all_pawn_objects if p.pawn_id == "professor"), None)
    ...
    for pawn in pawns_on_field:
        if pawn.pawn_id.startswith("player_"):
            if player_lives.get(pawn.pawn_id, 0) > 0:
                player_lives[pawn.pawn_id] -= 1; widget_to_shake = next((w for w in gameplay_screen.player_widgets if w.player_id == pawn.pawn_id), None)
                if widget_to_shake: widget_to_shake.start_lose_life_animation()
                print(f"!!! Profesor złapał gracza {pawn.pawn_id}! Gracz traci życie. Pozostało: {player_lives[pawn.pawn_id]}"); actual_change = add_points_to_player(pawn.pawn_id, -1); trigger_points_animation(pawn.pawn_id, actual_change)
                print(f"!!! Gracz {pawn.pawn_id} traci również 1 punkt ECTS."); update_player_widgets_data()
                if player_lives[pawn.pawn_id] <= settings.LIVES_TO_LOSE: print(f"!!! Gracz {pawn.pawn_id} stracił wszystkie życia!")
...
def start_pawn_move(roll_value):
    global turn_phase, pawn_that_was_moving; pawn_to_move = next((p for p in all_pawn_objects if p.pawn_id == current_player_id), None)
    ...
    for _ in range(roll_value): current_idx_in_path = (current_idx_in_path + 1) % total_fields; path_indices.append(current_idx_in_path)
...
def check_for_game_over():
    ...
    for i, name in enumerate(final_player_names):
        ...
        if player_points.get(player_id, 0) >= settings.POINTS_TO_WIN:
            print(f"ZWYCIĘSTWO! Gracz {name} zdobył {settings.POINTS_TO_WIN} ECTS!"); change_game_state("GAME_OVER", {"name": name, "reason": f"zdobył {settings.POINTS_TO_WIN} punktów ECTS!"}); return True
    loser_id = None
    for player_id, lives in player_lives.items():
        if lives <= settings.LIVES_TO_LOSE: loser_id = player_id; break
    if loser_id:
        loser_index = int(loser_id.split('_')[1]) - 1; loser_name = final_player_names[loser_index]
        winner_index = 1 if loser_index == 0 else 0; winner_name = final_player_names[winner_index]
        print(f"PORAŻKA! Gracz {loser_name} stracił wszystkie życia! Zwycięża {winner_name}!"); change_game_state("GAME_OVER", {"name": winner_name, "reason": f"przeciwnik stracił wszystkie życia."}); return True
    return False
...
def switch_turn():
    ...
    if current_player_id == "player_1": current_player_id = "player_2"
        ...
        global_turn_counter += 1; current_player_id = "player_1"
        if global_turn_counter > 0 and global_turn_counter % settings.PROFESSOR_BASE_MOVE_TURN_INTERVAL == 0:
            print(f"Minęły {settings.PROFESSOR_BASE_MOVE_TURN_INTERVAL} tury, profesor musi się ruszyć."); professor_move_pending += 1
...
def process_player_answer(chosen_answer_index):
    ...
    active_question_card.show_result(chosen_answer_index); correct_index = active_question_card.question_data["correct_answer_index"]
    if chosen_answer_index == correct_index:
        ...
        print("ODPOWIEDŹ BŁĘDNA!"); professor_move_pending += 1
...
def ask_next_exam_question():
    ...
    if exam_questions_left > 0:
        ...
        if question:
            exam_questions_left -= 1; subject_name = question.get("original_subject", "EGZAMIN")
        ...
        else: print("Brak więcej pytań w puli! Kończę egzamin."); end_exam_mode()
    else: end_exam_mode()
...
def end_exam_mode():
    ...
    if exam_correct_answers_count >= 2:
        ...
        if player_lives.get(current_player_id, 0) < settings.INITIAL_PLAYER_LIVES:
            ...
            player_lives[current_player_id] += 1; update_player_widgets_data()
            print(f"GRATULACJE! Gracz {current_player_id} zdobywa dodatkowe życie za zdany egzamin!")
        else: print(f"Gracz {current_player_id} zdał egzamin, ale ma już maksymalną liczbę żyć.")
    exam_mode_active = False; exam_correct_answers_count = 0; turn_phase = 'PROCESSING_AFTER_ACTION'
...
def handle_field_action():
    ...
    if field_data:
        ...
        if subject_name == "STYPENDIUM":
            print("Pole specjalne: STYPENDIUM! +2 ECTS"); points_to_animate = settings.POINTS_FOR_SCHOLARSHIP; turn_phase = 'PROCESSING_AFTER_ACTION'
        elif subject_name == "POPRAWKA":
            print("Pole specjalne: POPRAWKA! -2 ECTS"); points_to_animate = settings.POINTS_FOR_RETAKE; turn_phase = 'PROCESSING_AFTER_ACTION'
        elif subject_name == "EGZAMIN":
            print("Pole specjalne: EGZAMIN! Czas na 3 pytania."); exam_mode_active = True; exam_questions_left = 3; exam_correct_answers_count = 0; ask_next_exam_question()
        elif subject_name not in SPECIAL_FIELDS:
            question = question_manager.get_random_question_for_subject(subject_name)
            if question: active_question_card = QuestionCard(question, subject_name); turn_phase = 'SHOWING_QUESTION'
            else: print(f"Brak pytań dla: {subject_name}."); turn_phase = 'PROCESSING_AFTER_ACTION'
        else: print(f"Wylądowano na polu: {subject_name}."); turn_phase = 'PROCESSING_AFTER_ACTION'
    else: print(f"Brak danych dla pola {active_pawn.board_field_index}"); turn_phase = 'PROCESSING_AFTER_ACTION'

def forfeit_game():
    """Obsługuje poddanie gry przez aktywnego gracza."""
    loser_id = current_player_id
    try:
        loser_index = int(loser_id.split('_')[1]) - 1; loser_name = final_player_names[loser_index]
        winner_index = 1 if loser_index == 0 else 0; winner_name = final_player_names[winner_index]
        print(f"!!! PODDANIE GRY! Gracz {loser_name} poddaje grę! Zwycięża {winner_name}! !!!")
        change_game_state("GAME_OVER", {"name": winner_name, "reason": f"przeciwnik poddał grę."})
    except (IndexError, ValueError) as e:
        print(f"Błąd podczas poddawania gry: {e}"); change_game_state("MENU_GLOWNE")
...
def update_game_logic(dt_seconds, dice_instance):
    ...
    if turn_phase == 'DICE_ROLLING' and not dice_instance.is_animating:
        ...
        final_board_index = pawn_that_was_moving.board_field_index; total_fields = gameplay_screen.game_board_instance.get_total_fields(); roll_amount = dice_instance.current_roll; old_board_index = (final_board_index - roll_amount + total_fields) % total_fields
        remove_pawn_from_field_map(pawn_that_was_moving, old_board_index); add_pawn_to_field_map(pawn_that_was_moving, final_board_index)
        ...
        if not any_pawn_moving:
            if points_to_animate != 0:
                actual_change = add_points_to_player(current_player_id, points_to_animate); update_player_widgets_data(); trigger_points_animation(current_player_id, actual_change); points_to_animate = 0
            if professor_move_pending > 0:
                move_professor(professor_move_pending); professor_move_pending = 0
        ...
        if professor_that_was_moving and not professor_that_was_moving.is_moving:
            print("Profesor zakończył swój ruch."); check_professor_capture(); start_pawns_repositioning_on_field(professor_that_was_moving.board_field_index); professor_that_was_moving = None; turn_phase = 'PROCESSING_AFTER_ACTION'
        ...
        any_pawn_moving = next((p for p in all_pawn_objects if p.is_moving or p.is_repositioning), None)
        if not any_pawn_moving:
            if exam_mode_active: delay_timer = 0.0; turn_phase = 'INTER_QUESTION_DELAY'
            else: switch_turn()