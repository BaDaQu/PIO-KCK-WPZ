...
import pygame
import settings
import gameplay_screen  # Potrzebne do dostępu do game_board_instance
from pawn import PlayerPawn, ProfessorPawn

# --- Zmienne, które przeniesiono z main.py ---
...
def initialize_game_logic_and_pawns():
    """Resetuje logikę gry i inicjalizuje pionki. Wywoływana przy starcie gry."""
    ...
    if gameplay_screen.game_board_instance:
        try:
            ...
            print(f"Błąd krytyczny podczas inicjalizacji pionków w logice gry: {e}")
    else:
...
def start_pawns_repositioning_on_field(field_index):
    ...
    for pawn_obj in pawn_objects_on_field:
        pawn_obj.update_visual_position_on_field(field_index, pawn_ids_on_field)


def start_pawn_move(roll_value):
    """Rozpoczyna ruch dla aktualnego gracza."""
    ...
    if not pawn_to_move or not gameplay_screen.game_board_instance:
        print(f"Błąd: Nie można rozpocząć ruchu dla {current_player_id}.");
        switch_turn();
        return

    total_fields = gameplay_screen.game_board_instance.get_total_fields()
    if total_fields == 0 or roll_value == 0:
        print("Brak ruchu, przechodzę do następnej tury.");
        switch_turn();
        return
...
def switch_turn():
    """Przełącza turę na następnego gracza i resetuje fazę tury."""
...
def update_game_logic(dt_seconds, dice_instance):
    ...
    if turn_phase == 'PAWN_MOVING' and pawn_that_was_moving and not pawn_that_was_moving.is_moving:
        ...
        old_board_index = (
                                      final_board_index - dice_instance.current_roll + gameplay_screen.game_board_instance.get_total_fields()) % gameplay_screen.game_board_instance.get_total_fields()
    ...
    if turn_phase == 'FIELD_ACTION':
        ...
        if not any_pawn_repositioning:
            ...
            # TODO: Logika aktywacji pola (np. pytanie)
            switch_turn()