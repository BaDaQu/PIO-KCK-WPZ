...
class Pawn(pygame.sprite.Sprite):
    def __init__(self, image_path, initial_board_field_index, board_ref):
        ...
        if initial_center_pos:
            ...
            self.current_screen_pos_float = list(initial_center_pos)
        else:
            self.rect.topleft = (0, 0)
            self.current_screen_pos_float = [0.0, 0.0]
    ...
    def start_move_animation(self, path_of_indices):
        ...
        if self.current_screen_pos_float is None:
    ...
    def _set_next_target_from_path(self):
        if self.path:
            sound_manager.play_sound('pawn_move') # <-- Dźwięk na początku ruchu do nowego pola
            ...
            if not self.target_screen_center_pos:
                self.is_moving = False; self.path = []
        else:
            self.is_moving = False
            self.target_screen_center_pos = None
    ...
    def update_visual_position_on_field(self, field_index_param, pawns_on_this_field_ids):
        ...
        try: current_pawn_index_on_field = pawns_on_this_field_ids.index(self.pawn_id)
        ...
        if num_pawns_on_field == 2:
            ...
            if is_field_taller_than_wide:
                spacing = -5; total_area_height = self.pawn_render_height * 2 + spacing;
                start_y = field_screen_rect.centery - total_area_height // 2
                if current_pawn_index_on_field <= 0: target_pos[1] = start_y + self.pawn_render_height // 2
                else: target_pos[1] = start_y + self.pawn_render_height + spacing + self.pawn_render_height // 2
            else:
                spacing = self.pawn_render_width // 3; total_area_width = self.pawn_render_width * 2 + spacing;
                start_x = field_screen_rect.centerx - total_area_width // 2
                if current_pawn_index_on_field <= 0: target_pos[0] = start_x + self.pawn_render_width // 2
                else: target_pos[0] = start_x + self.pawn_render_width + spacing + self.pawn_render_width // 2
        elif num_pawns_on_field > 2:
            offset = 6; effective_index = current_pawn_index_on_field if current_pawn_index_on_field != -1 else 0
            target_pos[0] = field_screen_rect.centerx + int((effective_index - (num_pawns_on_field - 1) / 2) * offset)
            target_pos[1] = field_screen_rect.centery + int((effective_index - (num_pawns_on_field - 1) / 2) * offset / 2)
    ...
    def update(self, dt_seconds):
        ...
        if self.target_screen_center_pos and self.current_screen_pos_float:
            ...
            move_step = settings.PAWN_MOVE_SPEED_PIXELS_PER_SECOND * dt_seconds
            if distance <= move_step or distance < 1:
                ...
                dir_x = dx / distance if distance > 0 else 0; dir_y = dy / distance if distance > 0 else 0
