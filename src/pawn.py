...
class Pawn(pygame.sprite.Sprite):
    ...
    def _set_next_target_from_path(self):
        if self.path:
            ...
            print(f"Pionek {self.pawn_id}: Ustawiam cel na pole {next_field_index}")
            ...
            self.is_moving = False
            self.target_screen_center_pos = None
            print(f"Pionek {self.pawn_id}: Koniec ścieżki, zatrzymuję się na polu {self.board_field_index}.")

    def update_visual_position_on_field(self, field_index_param, pawns_on_this_field_ids):
        """Natychmiast ustawia pozycję wizualną pionka na polu, uwzględniając inne pionki."""
        ...
        if num_pawns_on_field == 2:
            ...
            if is_field_taller_than_wide:
                # --- ZMIANA TUTAJ ---
                # Zmniejszamy odstęp między pionkami
                spacing = 5  # Ustaw mały, stały odstęp w pikselach
                ...
                if current_pawn_index_on_field <= 0:
                    # Ustaw środek pionka na jego pozycji
        ...
        if pygame.math.Vector2(self.rect.center).distance_to(target_pos) > 1:
            ...
            print(f"Pionek {self.pawn_id}: Rozpoczynam rozsuwanie na polu {field_index_param}.")

    def update(self, dt):
        if not self.is_moving and not self.is_repositioning:
            return
        ...
        if self.target_screen_center_pos and self.current_screen_pos_float:
            ...
            if distance <= move_step or distance < 1:
                ...
                if self.is_moving:
                    ...
                    self.is_repositioning = False
                    self.target_screen_center_pos = None
                    print(f"Pionek {self.pawn_id}: Zakończono rozsuwanie.")
            else:
