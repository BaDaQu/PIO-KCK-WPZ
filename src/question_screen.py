...
def apply_rounded_corners_mask(image, radius):
    ...
    rounded_image.blit(image, (0, 0))  # Najpierw rysujemy obrazek
    rounded_image.blit(mask, (0, 0), special_flags=pygame.BLEND_RGBA_MIN)  # Potem wycinamy alfę za pomocą maski
...
class QuestionCard:
    ...
    def _load_assets(self):
        try:
            ...
            self.card_bg_image = apply_rounded_corners_mask(original_card_bg_image,
    ...
    # _setup_layout, handle_event, update_hover, update, show_result
    # POZOSTAJĄ BEZ ZMIAN W LOGICE (tak jak w Twojej ostatniej wersji)
    def _setup_layout(self):
        ...
        self.answer_buttons = []
        local_answer_rects = [settings.ANSWER_A_CLICK_RECT, settings.ANSWER_B_CLICK_RECT, settings.ANSWER_C_CLICK_RECT,
                              settings.ANSWER_D_CLICK_RECT]
    ...
    def draw(self, surface):
        ...
        text_utility.render_text_in_rect(surface=surface, text=self.question_data["question_text"],
        ...
        for i, answer_text in enumerate(self.question_data["answers"]):
            ...
            text_utility.render_text_in_rect(surface=surface, text=answer_text,
                                             font_path=settings.FONT_PATH_NOTO_SERIF_REGULAR,
