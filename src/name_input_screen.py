...
error_font = None
background_image = None  # Zmienna na dedykowane tło tego ekranu
...
def setup_name_input_screen(screen_width, screen_height):
    ...
    global input_boxes_rects, input_font, label_font, player_names_input, start_game_button, active_input_box_index, error_font, error_message, error_message_timer, background_image
    ...
    # --- NOWA LOGIKA: Ładujemy dedykowane tło dla tego ekranu ---
    try:
        ...
        background_image = None  # Jeśli się nie uda, użyjemy koloru fallback
    # --- KONIEC NOWEJ LOGIKI TŁA ---
    ...
    start_game_button = Button(
        ...
        text="Rozpocznij Wyścig!", font=menu_screen.BUTTON_FONT_MENU,  # Czcionka z menu pasuje stylistycznie
    ...
    if event.type == pygame.KEYDOWN:
        if active_input_box_index is not None:
            if event.key == pygame.K_BACKSPACE:
                ...
                # Blokujemy wpisywanie znaków, jeśli osiągnięto limit
                if len(player_names_input[active_input_box_index]) < settings.MAX_PLAYER_NAME_LENGTH:
                    ...
                    # Wyświetl komunikat o błędzie, jeśli gracz próbuje wpisać więcej znaków
...
def draw_name_input_screen(surface, screen_width, screen_height, mouse_pos, dt_seconds):
    ...
    # --- POPRAWIONA LOGIKA TŁA ---
    # Zawsze próbuj narysować dedykowane tło tego ekranu
    if background_image:
        surface.blit(background_image, (0, 0))
    else:
        # Jeśli się nie udało, użyj koloru fallback z settings
        surface.fill(settings.MENU_BG_FALLBACK_COLOR)
    # --- KONIEC POPRAWIONEJ LOGIKI TŁA ---

    # Rysowanie reszty UI
    # Używamy czcionki z menu do tytułu dla spójności
